const input = `FL7-|-FJ7.J.FF-.|7.JFF|F--7LJ-F--.--|.FF77LFL..7-F|-F|--L77F7-F7FFF--7---7FFL|7FJ7|J7.FF--F|7-F-7.L|FF|.FF7-J77F7FF7FF|.FF|7F7|-F.FJJFLJJFF-
...
const startingPoint = findS();
const array = [startingPoint];

const up = ['|', '7', 'F'].includes(
  map[startingPoint.row - 1][startingPoint.col]
);
// down
const down = ['|', 'L', 'J'].includes(
  map[startingPoint.row + 1][startingPoint.col]
);
// left
const left = ['-', 'L', 'F'].includes(
  map[startingPoint.row][startingPoint.col - 1]
);
// right
const right = ['-', '7', 'J'].includes(
  map[startingPoint.row][startingPoint.col + 1]
);
...
const pipeLoop = new Set([`${startingPoint.row},${startingPoint.col}`]);

while (array.length > 0) {
  const { col, row } = array.shift();
  const tile = map[row][col];
  for (const { col: nextCol, row: nextRow } of findNextPossiblePositions(
    row,
    col,
    tile
  )) {
    if (!pipeLoop.has(`${nextRow},${nextCol}`)) {
      array.push({ col: nextCol, row: nextRow });
      pipeLoop.add(`${nextRow},${nextCol}`);
    }
  }
}

let tilesWithinTheLoop = 0;
let fromTile;

for (let row = 0; row < map.length; row++) {
  let isInLoop = false;
  for (let col = 0; col < map[row].length; col++) {
    if (!pipeLoop.has(`${row},${col}`)) {
      if (isInLoop) {
        tilesWithinTheLoop++;
      }
    } else {
...
console.log(tilesWithinTheLoop);
...
function findNextPossiblePositions(row, col, tile) {
  switch (tile) {
    case '|':
      return [
        { row: row + 1, col },
...
function getTileBasedOnNeighbor({ up, down, left, right }) {
  if (up && down) return '|';
...
function setMapValue(map, row, col, val) {
  const str = map[row];
  map[row] = str.slice(0, col) + val + str.slice(col + 1);
}