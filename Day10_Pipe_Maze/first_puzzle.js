...
const array = [];
// up
if (['|', '7', 'F'].includes(map[startingPoint.row - 1][startingPoint.col])) {
  array.push({
    col: startingPoint.col,
    row: startingPoint.row - 1,
    distance: 1,
  });
}
// down
if (['|', 'L', 'J'].includes(map[startingPoint.row + 1][startingPoint.col])) {
  array.push({
    col: startingPoint.col,
    row: startingPoint.row + 1,
    distance: 1,
...
// left
if (['-', 'L', 'F'].includes(map[startingPoint.row][startingPoint.col - 1])) {
  array.push({
    col: startingPoint.col - 1,
    row: startingPoint.row,
    distance: 1,
  });
}
// right
if (['-', '7', 'J'].includes(map[startingPoint.row][startingPoint.col + 1])) {
  array.push({
    col: startingPoint.col + 1,
    row: startingPoint.row,
    distance: 1,
...
const pipeLoop = new Set([
  `${startingPoint.row},${startingPoint.col}`,
  `${array[0].row},${array[0].col}`,
  `${array[1].row},${array[1].col}`,
]);
let furthestDistance = 1;

while (array.length > 0) {
  const { col, row, distance } = array.shift();
  ...
  for (const { col: nextCol, row: nextRow } of findNextPossiblePositions(
    ...
    if (!pipeLoop.has(`${nextRow},${nextCol}`)) {
      array.push({ col: nextCol, row: nextRow, distance: distance + 1 });
